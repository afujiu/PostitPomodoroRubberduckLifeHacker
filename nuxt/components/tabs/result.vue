                ...
                <v-col class="my-0 mt-4"
                    ...
                    <div v-for="(val, uid) in list" :key="uid">
                        <span v-if="$plg.task.getTask(uid)!=null">
                            ...
                            <span :class="`${$plg.task.stateList[val.endState].color} ${$plg.task.stateList[val.endState].textColor}`">
                                {{$plg.task.stateList[val.endState].text }}
                                <span v-if="val.endState=='comp'">({{ $plg.util.mm(val.compTime) }}/{{$plg.util.dd(val.compTime)  }})</span>
                            </span>
                            <span v-if="isWrokTime">
                                {{$plg.util.msToMinute(val.sumWorkTime)}}:{{$plg.util.msToSecond(val.sumWorkTime)}}
                            </span>
                        </span>
                    </div>
                </v-col>
