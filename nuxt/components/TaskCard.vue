            ...
            <v-col cols="10">
              <v-row>
                ...
                <v-col cols="6" class="ma-0 pa-0 pl-0" :class="{ 'active-top': isContentsDial || isContentsDial }">
                      <!--子タスク作成-->
                      <v-btn fab x-small icon class="white--text blue darken-2 ml-2" elevation="10" @click="pushAddChild()">
                        <v-icon>mdi-expand-all-outline</v-icon>
                      </v-btn>
                      <!-- 小タスクの表示/非表示-->
                      <v-btn fab x-small icon class="white--text blue darken-2 ml-2" elevation="10" @click="isOpenChildlen=!isOpenChildlen">
